...
"""도서관 수와 도서 대출 수 사이에 어느 정도 상관관계가 있지만, 완전한 비례 관계는 아닙니다.
...
# 6. 디지털 문화 역세권인지에 따른 도서 대출량
"""
...
-> 도서관이 많다고 무조건 많은 것은 아니었다. 도서관 수와 도서 대출 수 사이에 어느 정도 상관관계가 있지만, 완전한 비례 관계는 아니다. 세종특별자치시는 가장 적은 도서관 수(24개)를 가지고 있지만, 도서관당 평균 대출 수는 가장 높다(7,080권).


**가설 2. 도서 대출 수는 학원 같은 교육 시설 수에 영향을 가장 많이 받을 것이다.**

->  가장 많이 받는 것이 아니었다. 상권업종과 도서관당 평균 대출 수 사이에는 약한 양의 상관관계가 있다. 상업 데이터 특성상 주거공간은 들어가지 않았는데 부동산이 높은 이유에는 주거공간이 근처 있음을 나타내는 것으로 보인다. 스포츠가 많은 이유는 도서관과 함께 스포츠 공간이 또 하나의 취미 생활의 공간이기 때문이라고 생각한다. 그 다음으로 학업이 뒤이은다. 그와 반대로 숙박은 음의 상관관계를 가졌다.

**가설 3. 문화시설 수가 많은 곳일 수록 도서 대출 수가 높을 것이다.**
  
  -> 생각보다 영향을 많이 주지 않는 듯 했다.
